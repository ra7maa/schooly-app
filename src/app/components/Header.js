    ...
    <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur-xl">
      ...
      <div className={`overflow-hidden border-t border-slate-200 bg-white/95 px-6 transition-all duration-300 sm:hidden ${open ? "max-h-[24rem] py-4 opacity-100" : "max-h-0 py-0 opacity-0"}`}>
        <nav className="flex flex-col gap-3 text-sm font-medium text-slate-700">
          <Link href="/" className="block rounded-2xl px-4 py-3 transition hover:bg-slate-100" onClick={() => setOpen(false)}>
            Home
          </Link>
          <Link href="/grades" className="block rounded-2xl px-4 py-3 transition hover:bg-slate-100" onClick={() => setOpen(false)}>
            Grades
          </Link>
          <Link href="/#contact" className="block rounded-2xl px-4 py-3 transition hover:bg-slate-100" onClick={() => setOpen(false)}>
            Contact Us
          </Link>
          <Link
            href="/profile"
            className="block rounded-2xl border border-[#4dadeb] bg-[#eef6ff] px-4 py-3 text-[#0f3b79] transition hover:bg-[#dfe9ff]"
