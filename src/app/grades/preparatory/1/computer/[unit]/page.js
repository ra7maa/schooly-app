      ...
      <div className="mx-auto grid max-w-7xl gap-8 xl:grid-cols-[1.2fr_0.8fr]">
        <section className="space-y-8 rounded-[2rem] border border-slate-200 bg-white p-10 shadow-lg shadow-slate-200/60">
          <div className="space-y-4">
            ...
            <h1 className="text-4xl font-semibold tracking-tight text-[#102d55] text-right">{unitData.title}</h1>
            <p className="max-w-3xl text-lg leading-8 text-slate-600 text-right">{unitData.subtitle}</p>
          </div>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {unitData.lessons.map((lesson) => (
              <Link
                ...
                className="rounded-[1.5rem] border border-slate-200 bg-[#f8fbff] p-6 transition hover:border-[#4dadeb] hover:bg-[#eef6ff] text-right"
              >
                <p className="text-sm font-semibold text-[#0f3b79]">{lesson.id.replace("lesson-", "الدرس ")}</p>
          ...
          <div className="rounded-[1.75rem] border border-[#4dadeb] bg-[#e8f2ff] p-6">
            <p className="text-sm font-semibold uppercase tracking-[0.24em] text-[#0f3b79] text-right">اختبار الوحدة النهائي</p>
            <p className="mt-3 text-slate-700 text-right">أنهِ هذه الوحدة باستخدام اختبار مراجعة مفيد.</p>
            <Link
            ...
            >
              انتقل إلى اختبار {unitData.title} النهائي
            </Link>
          </div>
        </section>
