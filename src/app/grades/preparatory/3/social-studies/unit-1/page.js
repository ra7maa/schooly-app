...
import { findUnit, socialStudiesUnits } from "../content";

const unit = findUnit("unit-1");
      ...
      <div className="mx-auto grid max-w-7xl gap-8 xl:grid-cols-[1.2fr_0.8fr]">
        <section className="space-y-8 rounded-[2rem] border border-slate-200 bg-white p-10 shadow-lg shadow-slate-200/60">
          <div className="space-y-4">
            ...
            <h1 className="text-4xl font-semibold tracking-tight text-[#102d55]">Social Studies Unit 1</h1>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {unit.lessons.map((lesson) => (
              <Link
                key={lesson.id}
                href={`/grades/preparatory/3/social-studies/unit-1/${lesson.id}/explanation`}
          ...
          <div className="rounded-[1.75rem] border border-[#4dadeb] bg-[#e8f2ff] p-6">
            ...
            <p className="mt-3 text-slate-700">Finish Unit 1 with a review exam that checks your understanding.</p>
            <Link
              href="/grades/preparatory/3/social-studies/unit-1/final-exam"
              className="mt-5 inline-flex rounded-full bg-[#0f3b79] px-4 py-2 text-sm font-semibold text-white hover:bg-[#0d336f]"
            >
              Go to Unit 1 Final Exam
            </Link>
          </div>
        </section>
